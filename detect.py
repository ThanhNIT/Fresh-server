...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                columns = ['cls','cf','imw','imh','x_min','y_min','x_max','y_max',]
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop or view_img:  # Add bbox to image
                        ...
                        record.extend(convertToCoordinate(float(array[1]),float(array[2]),float(array[3]),float(array[4]), array[6],array[7]))
                        if(len(record)==8): 
                            data.append(record)
    ...
    if(len(data_frame)==0):
        return f"{host}{source.replace('app','')}"
    ...
    for i in data_frame.index:
        ...
        img_crop = img[data_frame["y_min"][i]:data_frame["y_min"][i]+height, data_frame["x_min"][i]:data_frame["x_min"][i]+width]
        ...
        point = (int((data_frame['x_min'][i]+data_frame['x_max'][i])/2)-minus,int((data_frame['y_min'][i]+data_frame['y_max'][i])/2))
        img = cv2.putText(img,f"{className[predicted_class]}", point, cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2,cv2.LINE_AA, False)
        point = (int((data_frame['x_min'][i]+data_frame['x_max'][i])/2)-minus,int((data_frame['y_min'][i]+data_frame['y_max'][i])/2) +30)
        img = cv2.putText(img,f"{level}", point, cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2,cv2.LINE_AA, False)
...
def convertToCoordinate(x_rect_mid=0.0,y_rect_mid=0.0,width_rect=0.0,height_rect=0.0,img_width=0.0,img_height=0.0):
